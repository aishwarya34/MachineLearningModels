...
def forward_propagation(X, parameters):
    ...
    P3 = tf.nn.max_pool(A5, ksize = [1,7,7,1], strides = [1,4,4,1], padding = 'VALID')
    # FLATTEN
    P3 = tf.contrib.layers.flatten(P3)
    # FULLY-CONNECTED with non-linear activation function.
    A6 = tf.contrib.layers.fully_connected(P3, 4096, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
    A7 = tf.contrib.layers.fully_connected(A6, 100, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())    
    # FULLY-CONNECTED without non-linear activation function (not not call softmax).
    # 6 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    Z8 = tf.contrib.layers.fully_connected(A7, 6, activation_fn=None,  weights_initializer=tf.contrib.layers.xavier_initializer())
    ### END CODE HERE ###
...
_, _, parameters = model(X_train, Y_train, X_test, Y_test,  minibatch_size = 36)
